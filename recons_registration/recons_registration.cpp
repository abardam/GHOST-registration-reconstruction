...
#define SKELETON_ROTATION_CONFIDENCE 0.3
#define SKELETON_TRANSLATION_CONFIDENCE 1
#define SOURCE_POINT_THRESHOLD 3000
...
void estimate_skeleton_and_transform(
	...
	for (int i = 0; i < bpdv.size(); ++i){
		...
		cv::Mat source_bodypart_transform = get_bodypart_transform(bpdv[i], source_snhmap, source_camera_pose);
		cv::Mat target_bodypart_transform = get_bodypart_transform(bpdv[i], target_snhmap, target_camera_pose);
		...
		cv::Mat source_bodypart_pointmat;
		{
			...
			if (source_points.size() > SOURCE_POINT_THRESHOLD){
		...
		for (int iter = 0; iter<50; ++iter){ //straight up lets just do 50 iterations
			...
			if (!source_bodypart_pointmat.empty())
			{
				...
				if (false){
					point_to_plane_registration(source_bodypart_pointmat, source_depth, source_camera_matrix, source_bodypart_transform_inv,
						finalTransformDelta,
				...
				//point_to_plane_registration(source_bodypart_pointmat, source_depth, source_camera_matrix, source_parent_bodypart_transform_inv,
				//	target_depth, target_camera_matrix,
			...
			if (!source_bodypart_pointmat.empty())
			{
				...
				//point_to_point_registration(source_bodypart_pointmat, source_color, source_depth, source_camera_matrix, source_parent_bodypart_transform_inv,
				//	target_color, target_depth, target_camera_matrix, target_parent_bodypart_transform_inv, _A, _b, true);
			...
			//skeleton
			{
				...
				if (rel_skel){
					skeleton_constraints_linear(finalTransformDelta * source_node->mTransformation, target_node->mTransformation, SKELETON_ROTATION_CONFIDENCE * 1, SKELETON_TRANSLATION_CONFIDENCE * 1, _A, _b);
				}
				else{
			...
			//{
			//	cv::Vec3f rod;
			//	cv::Rodrigues(transformDelta, rod);
			//	cv::Rodrigues(rod, transformDelta(cv::Range(0, 3), cv::Range(0, 3)));
			//}
			...
			if (!debug_img.empty()){
				...
				for (int i = 0; i < bpdv.size(); ++i){
					...
					voxel_draw_volume(debug_img_volumes, color, source_camera_pose * finalTransformDelta * get_bodypart_transform(bpdv[i], source_snhmap, cv::Mat::eye(4, 4, CV_32F)), source_camera_matrix, &source_volumes[i], voxel_size);
				}
		...
		{
			cv::Vec3f rod;
			cv::Rodrigues(finalTransformDelta(cv::Range(0, 3), cv::Range(0, 3)), rod);
			cv::Rodrigues(rod, finalTransformDelta(cv::Range(0, 3), cv::Range(0, 3)));
		}
		if (rel_skel){
			node->mTransformation = finalTransformDelta * node->mTransformation;
			cv_draw_and_build_skeleton(node, node_parent->mTempTransformation, source_camera_matrix, source_camera_pose, &source_snhmap);
		}
		else{
	...
	for (int i = 0; i < bpdv.size(); ++i){
		...
		for (int iter = 0; iter<50; ++iter){ //straight up lets just do 50 iterations
			...
			//skeleton
			{
				...
				if (rel_skel){
					skeleton_constraints_linear(finalTransformDelta * source_bodypart_transform, target_bodypart_transform, SKELETON_ROTATION_CONFIDENCE * 1, SKELETON_TRANSLATION_CONFIDENCE * 1, _A, _b);
				}
				else{
					skeleton_constraints_linear(source_bodypart_transform, target_bodypart_transform, 1, 1, _A, _b);
				}
			...
			{
				cv::Vec3f rod;
				cv::Rodrigues(transformDelta(cv::Range(0, 3), cv::Range(0, 3)), rod);
				cv::Rodrigues(rod, transformDelta(cv::Range(0, 3), cv::Range(0, 3)));
			}
